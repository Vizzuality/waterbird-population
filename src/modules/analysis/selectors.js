...
export const selectFilteredData = createSelector(
  ...
  (_data, _filters, _publicationSelected) => {
    ...
    let publishedData = _data.map((d) => {
      const drafts = d.publications.filter((p) => p.published === 0).map((draft) => draft.id);

      const publishedOnePercentLevel = d.populationonepercentlevel.filter(
        (f) => !drafts.includes(f.publication_id)
      );
    ...
    const publicationData = publishedData.map((d) => {
      ...
      const selectedOnePercentLevel = publication
        ? d.populationonepercentlevel.filter((s) => s.publication_id === publication.id)
        : [];
      const selectedPublication = publication
        ? d.publications.filter((s) => s.id === publication.id)
        : [];
      const selectedSizes = publication
        ? d.sizes.filter((s) => s.publication_id === publication.id)
        : [];
      const selectedTrends = publication
        ? d.trends.filter((s) => s.publication_id === publication.id)
        : [];
...
export const selectFamilies = createSelector([selectFilteredData], (_data) => {
  if (!_data || isEmpty(_data)) return [];
...
export const selectFamilyTrends = createSelector(
  ...
  (_families, _data, _categories) => {
    if (!_data || isEmpty(_data)) return [];
    let populationsByFamily = _families.map((f) =>
      _data.filter((d) => trim(f.id) === trim(d.family.id) && d.trends.length)
    );
        ...
        populationsByFamily
          ...
          .map((p) => {
            ...
            const percentage = trendsCount.map((trend) => {
              return {
                [Object.keys(trend)]:
                  Object.values(trend)[0] !== 0 ? (Object.values(trend) * 100) / totalTrends : 0,
              };
            });
            return {
              id: p[0].family.id,
...
export const selectFamilyPopulations = createSelector(
  ...
  (_families, _data, _categories) => {
    ...
    const populationsByFamily = _families.map((f) =>
      _data.filter((d) => trim(f.id) === trim(d.family.id) && d.trends)
    );
...
export const selectFamilyTrendsChart = createSelector([selectFamilyTrends], (_familyTrends) => {
  return _familyTrends.map((d) => {
    const stable = d.percentage.find((s) => s['stable or fluctuating'])
      ? d.percentage.find((s) => s['stable or fluctuating'])['stable or fluctuating']
      : 0;
    const increasing = d.percentage.find((s) => s.increasing)
      ? d.percentage.find((s) => s.increasing).increasing
      : 0;
    const declining = d.percentage.find((s) => s.declining)
      ? d.percentage.find((s) => s.declining).declining
      : 0;
    const unknown = d.percentage.find((s) => s.unclear)
      ? d.percentage.find((s) => s.unclear).unclear
      : 0;
    const unclear = d.percentage.find((s) => s.unknown)
      ? d.percentage.find((s) => s.unknown).unknown
      : 0;
...
export const selectRegionTrendsChart = createSelector([selectFilteredData], (_data) => {
  const populationsByRegion = regions.map((r) => {
    const totalTrendsByRegion = {
      [r.name]: {
        name: _data.filter((d) => d[r.id] === 1).map((t) => (t.trends ? trim(t.name) : null)),

        trend: _data.filter((d) => d[r.id] === 1).map((t) => t.trends),
      },
    };
      ...
      [Object.keys(population)[0]]: {
        ...
        trend: {
          ...
          unknown: filteredTrends.reduce((n, t) => {
            return trim(t[0].state) === 'unknown' || trim(t[0].state) === 'unclear' ? n + 1 : n;
          }, 0),
...
export const selectGeneralData = createSelector(
  ...
  (_familyTrends, _familyPopulations, _regionTrendsChart) => {
    return _familyTrends.map((f) => {
      ...
      const increasing_percentage = f.percentage.filter(
        (p) => Object.keys(p)[0] === 'increasing'
      )[0];
      const stable_or_fluctuating_percentage = f.percentage.filter(
        (p) => Object.keys(p)[0] === 'stable or fluctuating'
      )[0];
      ...
      const unknown_total_percentage =
        Object.values(unclear_percentage)[0] &&
        Object.values(unknown_percentage)[0] &&
        Object.values(unclear_percentage)[0] + Object.values(unknown_percentage)[0];

      const pop_num_declining_trend = f.trendsCount.filter(
        (p) => Object.keys(p)[0] === 'declining'
      )[0];
      const pop_num_increasing_trend = f.trendsCount.filter(
        (p) => Object.keys(p)[0] === 'increasing'
      )[0];
      const pop_num_stable_or_fluctuating_trend = f.trendsCount.filter(
        (p) => Object.keys(p)[0] === 'stable or fluctuating'
      )[0];
      ...
      const pop_num_unknown_total_trend =
        Object.values(pop_num_unclear_trend)[0] &&
      ...
      const regions_neotropics = _regionTrendsChart.filter(
        (r) => Object.keys(r)[0] === 'Neotropics'
      )[0];
      ...
      const regions_north_america = _regionTrendsChart.filter(
        (r) => Object.keys(r)[0] === 'North America'
      )[0];
