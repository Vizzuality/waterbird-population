...
const FAQInfo = [
  {
    ...
    description: (
      <div>
        <p>No, the information on the Waterbird Populations Portal (WPP) is open access and freely available to all users. Registration is only required for a limited number of invited persons acting as coordinators or editors for a region or taxonomic group. For general users, registration will provide no additional benefits.</p>
      </div>
    )
  },
  {
  ...
  },
  {
    ...
    description: (
      <div>
        <p>Details on how to cite the website are provided at the bottom of the page and follow the format: Wetlands International ({year}) “Waterbird Populations Portal”. Retrieved from wpp.wetlands.org on {date}</p>
      </div>
    )
  },
  {
    ...
    description: (
      <div>
        <p>Earlier editions of the Waterbird Population Editions (WPE) are now out of print. All editions of WPE are still available digitally through this website at the <a href="/downloads/downloads">download page</a>. You can export search results (.csv format) and population fact-sheets (.pdf format) from this site by using the available buttons.</p>
      </div>
    ),
  },
  {
    ...
    description: (
      <div>
        <ol type="i">Please consider the following:
          ...
          <li>The species search field recognises partial string searches for both the common (English) and scientific name. Try searching for the genus or a part of the species name e.g. “Scoter” or “Larus”.</li>
  ...
  },
  {
    ...
    description: (
      <div>
        <p>Selecting a very large number of populations to export will cause the system to slow down and your download would either crash or take a very long time to complete. We therefore limit the number of records that can be downloaded in a single batch to improve the speed and efficiency of downloads for all users. You can download full publications, for example the Waterbird Populations Estimates 5 or the Conservation Status Review 7, from the <a href="/downloads/downloads">downloads page</a></p>
      </div>
    ),
  }
];
