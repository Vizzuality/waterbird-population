export const families = [
  {
    'id': '0',
    'name': 'Alcidae',
    'order_name': '',
  },
  {
    'id': '1',
    'name': 'Anatidae',
    'order_name': '',
  },
  {
    'id': '2',
    'name': 'Anseranatidae',
    'order_name': '',
  },
  {
    'id': '3',
    'name': 'Aramidae',
    'order_name': '',
  },
  ,
  {
    'id': '4',
    'name': 'Ardeidae',
    'order_name': '',
  },
  {
    'id': '5',
    'name': 'Balaenicipitidae',
    'order_name': '',
  },
  {
    'id': '6',
    'name': 'Burhinidae',
    'order_name': '',
  },
  ,
  {
    'id': '7',
    'name': 'Charadriidae',
    'order_name': '',
  },
  {
    'id': '8',
    'name': 'Ciconiidae',
    'order_name': '',
  },
  {
    'id': '9',
    'name': 'Dromadidae',
    'order_name': '',
  },
  {
    'id': '10',
    'name': 'Eurypygidae',
    'order_name': '',
  },
  {
    'id': '11',
    'name': 'Gaviidae',
    'order_name': '',
  },
  {
    'id': '12',
    'name': 'Glareolidae',
    'order_name': '',
  },
  {
    'id': '13',
    'name': 'Gruidae',
    'order_name': '',
  },
  {
    'id': '14',
    'name': 'Haematopodidae',
    'order_name': '',
  },
  {
    'id': '15',
    'name': 'Heliornithidae',
    'order_name': '',
  },
  {
    'id': '16',
    'name': 'Ibidorhynchidae',
    'order_name': '',
  },
  {
    'id': '17',
    'name': 'Jacanidae',
    'order_name': '',
  },
  {
    'id': '18',
    'name': 'Laridae',
    'order_name': '',
  },
  {
    'id': '19',
    'name': 'Pedionomidae',
    'order_name': '',
  },
  {
    'id': '20',
    'name': 'Pelecanidae',
    'order_name': '',
  },
  {
    'id': '21',
    'name': 'Phalacrocoracidae',
    'order_name': '',
  },
  {
    'id': '22',
    'name': 'Phoenicopteridae',
    'order_name': '',
  },
  {
    'id': '23',
    'name': 'Podicipedidae',
    'order_name': '',
  },
  {
    'id': '24',
    'name': 'Rallidae',
    'order_name': '',
  },
  {
    'id': '25',
    'name': 'Recurvirostridae',
    'order_name': '',
  },
  {
    'id': '26',
    'name': 'Rostratulidae',
    'order_name': '',
  },
  {
    'id': '27',
    'name': 'Scolopacidae',
    'order_name': '',
  },
  {
    'id': '28',
    'name': 'Spheniscidae',
    'order_name': '',
  },
  {
    'id': '29',
    'name': 'Sulidae',
    'order_name': '',
  },
  {
    'id': '30',
    'name': 'Thinocoridae',
    'order_name': '',
  },
  {
    'id': '31',
    'name': 'Threskiornithidae',
    'order_name': '',
  }
];

export const species = [
  {
    'id': 0,
    'common_name': 'African Woollyneck',
    'scientific_name': 'Ciconia microscelis',
    'genus': 'Ciconia',
    'family_id': 12,
    'description': 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam hendrerit dolor mi. Nullam viverra lectus felis, eget laoreet orci luctus ac. Donec quam ex, ornare et feugiat id, convallis sed diam. Morbi mollis vulputate est aliquam consectetur. Pellentesque ultricies vulputate diam. Vivamus finibus varius placerat. Etiam ullamcorper ex enim, a iaculis urna malesuada nec. Ut sed elit consectetur, pulvinar dui eleifend, scelerisque ante. Nulla efficitur ullamcorper ante non varius. Aliquam ac mattis metus, id maximus orci. Curabitur iaculis congue erat. Ut dictum viverra nulla, sit amet vehicula velit facilisis at.',
    'red_list_id': 4, // table(red list category): description??
  },
  {
    'id': 1,
    'common_name': 'Black Stork',
    'scientific_name': 'Ciconia nigra',
    'genus': 'Ciconia',
    'family_id': 12,
    'description': 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam hendrerit dolor mi. Nullam viverra lectus felis, eget laoreet orci luctus ac. Donec quam ex, ornare et feugiat id, convallis sed diam. Morbi mollis vulputate est aliquam consectetur. Pellentesque ultricies vulputate diam. Vivamus finibus varius placerat. Etiam ullamcorper ex enim, a iaculis urna malesuada nec. Ut sed elit consectetur, pulvinar dui eleifend, scelerisque ante. Nulla efficitur ullamcorper ante non varius. Aliquam ac mattis metus, id maximus orci. Curabitur iaculis congue erat. Ut dictum viverra nulla, sit amet vehicula velit facilisis at.',
    'red_list_id': 1,
  },  {
    'id': 2,
    'common_name': 'Marabou',
    'scientific_name': 'Leptoptilos crumenifer',
    'genus': 'Leptoptilos',
    'family_id': 12,
    'description': 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam hendrerit dolor mi. Nullam viverra lectus felis, eget laoreet orci luctus ac. Donec quam ex, ornare et feugiat id, convallis sed diam. Morbi mollis vulputate est aliquam consectetur. Pellentesque ultricies vulputate diam. Vivamus finibus varius placerat. Etiam ullamcorper ex enim, a iaculis urna malesuada nec. Ut sed elit consectetur, pulvinar dui eleifend, scelerisque ante. Nulla efficitur ullamcorper ante non varius. Aliquam ac mattis metus, id maximus orci. Curabitur iaculis congue erat. Ut dictum viverra nulla, sit amet vehicula velit facilisis at.',
    'red_list_id': 2,
  },
  {
    'id': 3,
    'common_name': ,
    'scientific_name': 'Ciconia-nigra',
    'genus': '',
    'family_id': 9,
    'description': 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam hendrerit dolor mi. Nullam viverra lectus felis, eget laoreet orci luctus ac. Donec quam ex, ornare et feugiat id, convallis sed diam. Morbi mollis vulputate est aliquam consectetur. Pellentesque ultricies vulputate diam. Vivamus finibus varius placerat. Etiam ullamcorper ex enim, a iaculis urna malesuada nec. Ut sed elit consectetur, pulvinar dui eleifend, scelerisque ante. Nulla efficitur ullamcorper ante non varius. Aliquam ac mattis metus, id maximus orci. Curabitur iaculis congue erat. Ut dictum viverra nulla, sit amet vehicula velit facilisis at.',
    'red_list_id': 4,
  },
  {
    'id': 4,
    'common_name': 'Yellow-billed Stork',
    'scientific_name': 'Mycteria ibis',
    'genus': 'Mycteria',
    'family_id': 20,
    'description': 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam hendrerit dolor mi. Nullam viverra lectus felis, eget laoreet orci luctus ac. Donec quam ex, ornare et feugiat id, convallis sed diam. Morbi mollis vulputate est aliquam consectetur. Pellentesque ultricies vulputate diam. Vivamus finibus varius placerat. Etiam ullamcorper ex enim, a iaculis urna malesuada nec. Ut sed elit consectetur, pulvinar dui eleifend, scelerisque ante. Nulla efficitur ullamcorper ante non varius. Aliquam ac mattis metus, id maximus orci. Curabitur iaculis congue erat. Ut dictum viverra nulla, sit amet vehicula velit facilisis at.',
    'red_list_id': 3,
  },
  {
    'id': 5,
    'common_name': 'Crab-plover',
    'scientific_name': 'Dromas ardeola',
    'genus': 'Dromas',
    'family_id': 4,
    'description': 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam hendrerit dolor mi. Nullam viverra lectus felis, eget laoreet orci luctus ac. Donec quam ex, ornare et feugiat id, convallis sed diam. Morbi mollis vulputate est aliquam consectetur. Pellentesque ultricies vulputate diam. Vivamus finibus varius placerat. Etiam ullamcorper ex enim, a iaculis urna malesuada nec. Ut sed elit consectetur, pulvinar dui eleifend, scelerisque ante. Nulla efficitur ullamcorper ante non varius. Aliquam ac mattis metus, id maximus orci. Curabitur iaculis congue erat. Ut dictum viverra nulla, sit amet vehicula velit facilisis at.',
    'red_list_id': 4,
  },  {
    'id': 6,
    'common_name': 'Lesser Frigatebird',
    'scientific_name': 'Fregata ariel',
    'genus': 'Fregata',
    'family_id': 9,
    'description': 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam hendrerit dolor mi. Nullam viverra lectus felis, eget laoreet orci luctus ac. Donec quam ex, ornare et feugiat id, convallis sed diam. Morbi mollis vulputate est aliquam consectetur. Pellentesque ultricies vulputate diam. Vivamus finibus varius placerat. Etiam ullamcorper ex enim, a iaculis urna malesuada nec. Ut sed elit consectetur, pulvinar dui eleifend, scelerisque ante. Nulla efficitur ullamcorper ante non varius. Aliquam ac mattis metus, id maximus orci. Curabitur iaculis congue erat. Ut dictum viverra nulla, sit amet vehicula velit facilisis at.',
    'red_list_id': 1,
  },
  {
    'id': 7,
    'common_name': 'Great Frigatebird',
    'scientific_name': 'Fregata minor',
    'genus': 'Fregata',
    'family_id': 12,
    'description': 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam hendrerit dolor mi. Nullam viverra lectus felis, eget laoreet orci luctus ac. Donec quam ex, ornare et feugiat id, convallis sed diam. Morbi mollis vulputate est aliquam consectetur. Pellentesque ultricies vulputate diam. Vivamus finibus varius placerat. Etiam ullamcorper ex enim, a iaculis urna malesuada nec. Ut sed elit consectetur, pulvinar dui eleifend, scelerisque ante. Nulla efficitur ullamcorper ante non varius. Aliquam ac mattis metus, id maximus orci. Curabitur iaculis congue erat. Ut dictum viverra nulla, sit amet vehicula velit facilisis at.',
    'red_list_id': 3,
  },
];

export const populations = [
  {
    'id': '',
    // *tables*
      // table: pop 1%level
      // table (population publication): publication id, pop trend id, pop size id, 1% id, id
        // from publication id-> table (publication): description??
        // from pop trend id:
            //table (pop trend ref): reference (from notes?)
            //table (pop trend): trend start-end year, notes for name?, trend quaility of,
        // from pop size id-> table (pop size): pop start-end year, maximun-minimun, estimate quality id
            //from estimate quality id: estimate quality description??
        // from 1% id -> table(pop 1% level): id, year set, one percent, notes
          // from id: table (publication population): publication id, pop trend id, pop size id, 1% id, id
            // from publication id-> table (publication): description??

    'name': '',
    'number': '',
    'breeding_range': '',
    'non_breeding_range': '',
    'family_id': '',
    'species_id': '',
    'trend_id': '',
    // *tables*
      // --table(pop trend): quality, code, trend id
          //from trend id->
            //table (pop trend ref): reference (from notes?)
            //table (pop trend): trend start-end year, notes for name?, trend quaility of,
              //from trend quality id-> table (trend quality code): description (is needed?)
  }
]
